...
public class JLANServerService implements WrapperListener, Runnable {
    ...
    public Integer start(String[] args) {
        ...
        try {
            ...
            String mornitorname = System.getProperty("mount_monitor", "monitor.conf");
            File f = new File(mornitorname);
            if (f.exists() && f.isFile()) {
                try {
                    ...
                    if (!(classname == null || classname.trim().isEmpty())) {
                        ...
                        mornitor = (Monitor) cls.getConstructor(new Class[]{Properties.class}).newInstance(p);
                        mornitor.start();
                    }
                } catch (Exception ex) {
                }
    ...
    protected final static NetworkServer createFTPServer(ServerConfiguration config)
        ...
        return createServer(classname, config);
    }
    ...
    protected final static NetworkServer createNFSServer(ServerConfiguration config)
            throws Exception {
        String classname = NFSServer.class.getName();
    ...
    protected final static NetworkServer createNFSMountServer(ServerConfiguration config)
            throws Exception {
        String classname = MountServer.class.getName();
     ...
     * @return NetworkServer
     */
    protected final static NetworkServer createNFSPortMapper(ServerConfiguration config) throws Exception {
        String classname = PortMapperServer.class.getName();
     ...
     * @exception Exception
     */
    protected final static NetworkServer createServer(String className, ServerConfiguration config) throws Exception {
    ...
    public void run() {
        ...
        if (m_config.numberOfServers() > 0) {
            ...
            for (int i = 0; i < m_config.numberOfServers(); i++) {
                ...
                //	DEBUG
                LogFactory.info("Starting server " + server.getProtocolName() + " ...", JLANServerService.class);
