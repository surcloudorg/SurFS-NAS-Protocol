...
public abstract class NetworkFile {
    ...
    // File size
    protected AtomicLong m_fileSize = new AtomicLong(0);

    // File creation/modify/last access date/time
    protected long m_createDate;
    protected AtomicLong m_modifyDate = new AtomicLong(0);
    ...
    protected AtomicBoolean m_closed = new AtomicBoolean(true);

    // Count of write requests to the file, used to determine if the file size may have changed
    protected AtomicInteger m_writeCount = new AtomicInteger(0);
    ...
    public final int getFileSizeInt() {
        return (int) (m_fileSize.get() & 0x0FFFFFFFFL);
    }
    ...
    public final boolean hasLocks() {
        return m_lockList != null && m_lockList.numberOfLocks() > 0;
    }
    ...
    public final boolean hasDelayedWriteError() {
        return (m_flags & DelayedWriteError) != 0;
    }
    ...
    public final boolean hasDeleteOnClose() {
        return (m_flags & DeleteOnClose) != 0;
    }
