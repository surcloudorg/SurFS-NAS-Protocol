...
public class NFSServer extends RpcNetworkServer implements RpcProcessor {
    ...
    public final static int MaxRequestSize = 1024 * 1024 - 1;
    ...
    private static final int DefaultPacketPoolSize = 24;
     ...
     * @return RpcPacket
     */
    private RpcPacket procNull(NFSSrvSession sess, RpcPacket rpc) {
        //	Build the response
     ...
     * @return RpcPacket
     */
    private RpcPacket procGetAttr(NFSSrvSession sess, RpcPacket rpc) {
     ...
     * @return RpcPacket
     */
    private RpcPacket procSetAttr(NFSSrvSession sess, RpcPacket rpc) {
        ...
        try {
            ...
            if (fsize != -1L) {
                ...
                //}
                //	DEBUG
                newInfo = netFile.getFileInformation();
                if (newInfo == null) {
                    newInfo = disk.getFileInformation(sess, conn, netFile.getFullName());
                }
                if (Debug.EnableInfo && hasDebugFlag(DBG_INFO)) {
                    sess.debugPrintln("SetAttr handle=" + NFSHandle.asString(handle) + ", newSize=" + fsize);
                }
            } else {
                newInfo = disk.getFileInformation(sess, conn, path);
            }
            //	Pack the response
            rpc.buildResponseHeader();
            rpc.packInt(NFS.StsSuccess);
     ...
     * @return RpcPacket
     */
    private RpcPacket procLookup(NFSSrvSession sess, RpcPacket rpc) {
     ...
     * @return RpcPacket
     */
    private RpcPacket procAccess(NFSSrvSession sess, RpcPacket rpc) {
     ...
     * @return RpcPacket
     */
    private RpcPacket procReadLink(NFSSrvSession sess, RpcPacket rpc) {
     ...
     * @return RpcPacket
     */
    private RpcPacket procRead(NFSSrvSession sess, RpcPacket rpc) {
        ...
        try {
            ...
            FileInfo finfo = netFile.getFileInformation();
            if (finfo == null) {
                finfo = disk.getFileInformation(sess, conn, netFile.getFullName());
            }
            packPostOpAttr(sess, finfo, shareId, rpc);
     ...
     * @return RpcPacket
     */
    private RpcPacket procWrite(NFSSrvSession sess, RpcPacket rpc) {
        //	Unpack the read parameters
        byte[] handle = new byte[NFS.FileHandleSize];
        rpc.unpackByteArrayWithLength(handle);
        long offset = rpc.unpackLong();
        ...
        try {
            ...
            preInfo = netFile.getFileInformation();
            if (preInfo == null) {
                preInfo = disk.getFileInformation(sess, conn, path);
            }
            ...
            FileInfo finfo = netFile.getFileInformation();
            if (finfo == null) {
                finfo = disk.getFileInformation(sess, conn, path);
            }
            rpc.buildResponseHeader();
            rpc.packInt(NFS.StsSuccess);
     ...
     * @return RpcPacket
     */
    private RpcPacket procCreate(NFSSrvSession sess, RpcPacket rpc) {
     ...
     * @return RpcPacket
     */
    private RpcPacket procMkDir(NFSSrvSession sess, RpcPacket rpc) {
     ...
     * @return RpcPacket
     */
    private RpcPacket procSymLink(NFSSrvSession sess, RpcPacket rpc) {
     ...
     * @return RpcPacket
     */
    private RpcPacket procMkNode(NFSSrvSession sess, RpcPacket rpc) {
        //	DEBUG
        if (Debug.EnableInfo && hasDebugFlag(DBG_DIRECTORY)) {
     ...
     * @return RpcPacket
     */
    private RpcPacket procRemove(NFSSrvSession sess, RpcPacket rpc) {
     ...
     * @return RpcPacket
     */
    private RpcPacket procRmDir(NFSSrvSession sess, RpcPacket rpc) {
     ...
     * @return RpcPacket
     */
    private RpcPacket procRename(NFSSrvSession sess, RpcPacket rpc) {
     ...
     * @return RpcPacket
     */
    private RpcPacket procLink(NFSSrvSession sess, RpcPacket rpc) {
        //	DEBUG
     ...
     * @return RpcPacket
     */
    private RpcPacket procReadDir(NFSSrvSession sess, RpcPacket rpc) {
     ...
     * @return RpcPacket
     */
    private RpcPacket procReadDirPlus(NFSSrvSession sess, RpcPacket rpc) {
        //	Unpack the read directory arguments
        byte[] handle = new byte[NFS.FileHandleSize];
        rpc.unpackByteArrayWithLength(handle);
        long cookie = rpc.unpackLong();
        ...
        try {
            ...
            while (entCnt++ < maxDir && replyFull == false && search.nextFileInfo(finfo)) {
                //	Check if the new file entry will fit into the reply buffer without exceeding the clients maximum
                //	reply size
                int fnamesize = finfo.getFileName().getBytes().length;
                int entryLen = READDIRPLUS_ENTRY_LENGTH + ((fnamesize + 3) & 0xFFFFFFFC);
     ...
     * @return RpcPacket
     */
    private RpcPacket procFsStat(NFSSrvSession sess, RpcPacket rpc) {
        //	Get the handle from the request
        byte[] handle = new byte[NFS.FileHandleSize];
        rpc.unpackByteArrayWithLength(handle);
        //	DEBUG
     ...
     * @return RpcPacket
     */
    private RpcPacket procFsInfo(NFSSrvSession sess, RpcPacket rpc) {
        //	Get the handle from the request
        byte[] handle = new byte[NFS.FileHandleSize];
        rpc.unpackByteArrayWithLength(handle);
        //	DEBUG
        ...
        try {
            ...
            rpc.packInt(NFSInfo.getMaxSize());
     ...
     * @return RpcPacket
     */
    private RpcPacket procPathConf(NFSSrvSession sess, RpcPacket rpc) {
     ...
     * @return RpcPacket
     */
    private RpcPacket procCommit(NFSSrvSession sess, RpcPacket rpc) {
        //	DEBUG
        if (Debug.EnableInfo && hasDebugFlag(DBG_FILEIO)) {
     ...
     */
    private NFSSrvSession findSessionForRequest(RpcPacket rpc) throws RpcAuthenticationException {
     ...
     * @return NFSSrvSession
     */
    private NFSSrvSession findAuthNullSession(RpcPacket rpc, Object sessKey) {
     ...
     * @return NFSSrvSession
     */
    private NFSSrvSession findAuthUnixSession(RpcPacket rpc, Object sessKey) {
    ...
    protected final boolean pathHasDirectories(String fpath) {
        ...
        return fpath.contains("\\") || fpath.contains("/");
    }
