 ...
 *
 * <p>
 * Receives session requests via a TCP socketRPC requests via a datagram and
 * passes the request to the registered RPC server.
 *
 * @author gkspencer
 */
public class TcpRpcSessionHandler extends SocketSessionHandler {
    ...
    private final HashMap<Integer, TcpRpcPacketHandler> m_sessions;
    ...
    protected void acceptConnection(Socket sock) {
        try {
            ...
            m_sessions.put(sessId, pktHandler);
            //	DEBUG
    ...
    public void closeSessionHandler(NetworkServer server) {
        ...
        if (m_sessions.size() > 0) {
            ...
            List<TcpRpcPacketHandler> enm = new ArrayList<>(m_sessions.values());
            for (PacketHandlerInterface handler : enm) {
