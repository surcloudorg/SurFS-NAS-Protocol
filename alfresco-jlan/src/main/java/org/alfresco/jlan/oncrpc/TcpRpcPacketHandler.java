...
public class TcpRpcPacketHandler extends SocketPacketHandler implements Runnable {
    ...
    protected final int receiveRpc(byte[] buffer, int offset, int maxLen)
        ...
        while (lastFrag == false) {
            ...
            while (ddlen > 0) {
                // Read the data
                len = readPacket(m_fragBuf, off, ddlen);
            ...
            if (fragLen > (buffer.length - rxOffset)) {
                throw new SocketException("Receive RPC buffer overflow, fragment len = "
                        + fragLen + ",buflen=" + buffer.length + ",offset=" + rxOffset);
            }
    ...
    /**
     * Allocate an RPC packet for receiving an incoming request. This method
     * must be overridden for multi-threaded implementations.
     *
     * @param maxSize int
     * @return RpcPacket
     */
    ...
    /**
     * Deallocate an RPC packet, default method does nothing but a pooled
     * implementation may return the packet to the pool.
     *
     * @param pkt RpcPacket
     */
    ...
    /**
     * Process an RPC request. This method must be overridden for multi-threaded
     * implementations.
     *
     * @param rpc RpcPacket
     * @exception IOException
