...
public class TcpipSMBSessionSocketHandler extends SessionSocketHandler {
    ...
    public void run() {
        try {
            ...
            while (hasShutdown() == false) {
                ...
                //  Debug
                if (Debug.EnableInfo && hasDebug()) {
                    Debug.println("[SMB] TCP-SMB session request received from " + sessSock.socket().getInetAddress().getHostAddress());
                }
                try {
                    //Create a packet handler for the session
                    PacketHandler pktHandler = new TcpipSMBPacketHandler(sessSock.socket());
                    ...
                    srvThread.setName("Sess_T" + srvSess.getSessionId() + "_" + sessSock.socket().getInetAddress().getHostAddress());
                    srvThread.start();
                } catch (Exception ex) {
                    //Debug
                    if (Debug.EnableInfo && hasDebug()) {
    ...
    /**
     * Create the TCP/IP native SMB/CIFS session socket handlers for the main
     * SMB/CIFS server
     *
