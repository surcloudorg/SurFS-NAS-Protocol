...
public class UUID {
    ...
    public static final String generateUUIDString(byte[] buf) {
        ...
        StringBuilder str = new StringBuilder(UUID_LENGTH);
    ...
    public final int storeUUID(byte[] buf, int off, boolean writeVer) {
        ...
        System.arraycopy(uuidByts, 0, buf, pos, UUID_LENGTH_BINARY);
     ...
     * ( u.isValid()) { System.out.println("" + (i+1) + ": " + u.toString());
     * byte[] bytes = u.getBytes(); HexDump.Dump(bytes,bytes.length, 0);
