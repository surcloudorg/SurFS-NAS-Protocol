...
public class SMBServer extends NetworkFileServer implements Runnable, ConfigurationListener {
    ...
    private List<SessionSocketHandler> m_sessionHandlers;
    ...
    private final int m_srvType = ServerType.WorkStation + ServerType.Server;
    ...
    public final void addSessionHandler(SessionSocketHandler handler) {
        ...
        if (m_sessionHandlers == null) {
            m_sessionHandlers = new LinkedList<>();
        }
    ...
    public final void addHostAnnouncer(HostAnnouncer announcer) {
        ...
        if (m_hostAnnouncers == null) {
            m_hostAnnouncers = new LinkedList<>();
        }
    ...
    protected void closeHostAnnouncers() {
        ...
        if (m_hostAnnouncers != null) {
            while (!m_hostAnnouncers.isEmpty()) {
                HostAnnouncer announcer = (HostAnnouncer) m_hostAnnouncers.remove(0);
    ...
    protected void closeSessionHandlers() {
        if (m_sessionHandlers != null) {
            while (!m_sessionHandlers.isEmpty()) {
                SessionSocketHandler handler = (SessionSocketHandler) m_sessionHandlers.remove(0);
        ...
        try {
            ...
            switch (id) {
                ...
                case ConfigId.ServerSMBEnable:
                    ...
                    if (isActive() && enaSMB == false) {
                        //	Shutdown the server
                        shutdownServer(false);
                    } else if (isActive() == false && enaSMB == true) {
                        //	Start the server
                        startServer();
                    }
                ...
                case ConfigId.SMBServerType:
                    ...
                    if (m_hostAnnouncers != null) {
                        ...
                        for (HostAnnouncer m_hostAnnouncer : m_hostAnnouncers) {
                            HostAnnouncer announcer = (HostAnnouncer) m_hostAnnouncer;
     ...
     * @return boolean
     */
    private boolean isWindowsNTOnwards() {
     ...
     *
     */
    private void getServerIPAddresses() {
        try {
